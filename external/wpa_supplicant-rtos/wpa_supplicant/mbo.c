...
const u8 *mbo_get_attr_from_ies(const u8 *ies, size_t ies_len, enum mbo_attr_id attr)
{
    const u8 *mbo_ie;
...
void wpas_mbo_check_pmf(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    if (wpa_s->disable_mbo_oce)
        wpa_printf(MSG_INFO, "MBO: Disable MBO/OCE due to misbehaving AP not having enabled PMF");
}

static void wpas_mbo_non_pref_chan_attr_body(struct wpa_supplicant *wpa_s, struct wpabuf *mbo, u8 start, u8 end)
{
    u8 i;
...
static void wpas_mbo_non_pref_chan_attr(struct wpa_supplicant *wpa_s, struct wpabuf *mbo, u8 start, u8 end)
{
...
static void wpas_mbo_non_pref_chan_subelement(struct wpa_supplicant *wpa_s, struct wpabuf *mbo, u8 start, u8 end)
{
...
static void wpas_mbo_non_pref_chan_attrs(struct wpa_supplicant *wpa_s, struct wpabuf *mbo, int subelement)
{
    u8 i, start = 0;
    ...
    for (i = 1; i <= wpa_s->non_pref_chan_num; i++)
    {
        ...
        if (!non_pref || non_pref->oper_class != start_pref->oper_class || non_pref->reason != start_pref->reason ||
            non_pref->preference != start_pref->preference)
        {
            if (subelement)
                wpas_mbo_non_pref_chan_subelement(wpa_s, mbo, start, i);
            else
...
int wpas_mbo_ie(struct wpa_supplicant *wpa_s, u8 *buf, size_t len, int add_oce_capa)
{
    ...
    if (len < MBO_IE_HEADER + 3 + 7 + ((wpa_s->enable_oce & OCE_STA) ? 3 : 0))
        return 0;
...
static void wpas_mbo_send_wnm_notification(struct wpa_supplicant *wpa_s, const u8 *data, size_t len)
{
    struct wpabuf *buf;
    int res;
    ...
    res = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid,
                              wpabuf_head(buf), wpabuf_len(buf), 0);
    if (res < 0)
        wpa_printf(MSG_DEBUG, "Failed to send WNM-Notification Request frame with non-preferred channel list");
...
static void wpas_mbo_non_pref_chan_changed(struct wpa_supplicant *wpa_s)
{
    ...
    wpas_mbo_send_wnm_notification(wpa_s, wpabuf_head_u8(buf), wpabuf_len(buf));
    wpas_update_mbo_connect_params(wpa_s);
    wpabuf_free(buf);
}

static int wpa_non_pref_chan_is_eq(struct wpa_mbo_non_pref_channel *a, struct wpa_mbo_non_pref_channel *b)
{
...
int wpas_mbo_update_non_pref_chan(struct wpa_supplicant *wpa_s, const char *non_pref_chan)
{
    ...
    while ((token = str_token(cmd, " ", &context)))
    {
        ...
        if (num == size)
        {
            ...
            if (!tmp_chans)
            {
                wpa_printf(MSG_ERROR, "Couldn't reallocate non_pref_chan");
                goto fail;
            }
            chans = tmp_chans;
        }
        ...
        ret = sscanf(token, "%u:%u:%u:%u", &_oper_class, &_chan, &_preference, &_reason);
        if (ret != 4 || _oper_class > 255 || _chan > 255 || _preference > 255 || _reason > 65535)
        {
            wpa_printf(MSG_ERROR, "Invalid non-pref chan input %s", token);
            goto fail;
        }
        ...
        if (wpas_mbo_validate_non_pref_chan(chan->oper_class, chan->chan, chan->reason))
        {
            wpa_printf(MSG_ERROR, "Invalid non_pref_chan: oper class %d chan %d reason %d", chan->oper_class,
        ...
        if (i != num)
        {
            wpa_printf(MSG_ERROR, "oper class %d chan %d is duplicated", chan->oper_class, chan->chan);
            goto fail;
        }
    ...
    if (chans)
    {
        qsort(chans, num, sizeof(struct wpa_mbo_non_pref_channel), wpa_non_pref_chan_cmp);
    }
...
void wpas_mbo_ie_trans_req(struct wpa_supplicant *wpa_s, const u8 *mbo_ie, size_t len)
{
    const u8 *pos, *cell_pref = NULL;
    u8 id, elen;
    u16 disallowed_sec = 0;

    if (len <= 4 || WPA_GET_BE24(mbo_ie) != OUI_WFA || mbo_ie[3] != MBO_OUI_TYPE)
        return;
    ...
    while (len >= 2)
    {
        ...
        switch (id)
        {
            case MBO_ATTR_ID_CELL_DATA_PREF:
                ...
                if (wpa_s->conf->mbo_cell_capa == MBO_CELL_CAPA_AVAILABLE)
                    cell_pref = pos;
                else
                    wpa_printf(MSG_DEBUG, "MBO: Station does not support Cellular data connection");
                break;
            ...
            case MBO_ATTR_ID_ASSOC_RETRY_DELAY:
                ...
                {
                    wpa_printf(MSG_DEBUG, "MBO: Unexpected association retry delay, BSS is terminating");
                    goto fail;
                }
                else if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_DISASSOC_IMMINENT)
                {
                    disallowed_sec = WPA_GET_LE16(pos);
                    wpa_printf(MSG_DEBUG, "MBO: Association retry delay: %u", disallowed_sec);
                }
                else
                {
                    wpa_printf(MSG_DEBUG, "MBO: Association retry delay attribute not in disassoc imminent mode");
                }
            ...
            case MBO_ATTR_ID_TRANSITION_REJECT_REASON:
                wpa_printf(MSG_DEBUG, "MBO: Attribute %d should not be included in BTM Request frame", id);
                break;
            default:
                wpa_printf(MSG_DEBUG, "MBO: Unknown attribute id %u", id);
                return;
        }
    ...
    if (cell_pref)
        wpa_msg(wpa_s, MSG_INFO, MBO_CELL_PREFERENCE "preference=%u", *cell_pref);

    if (wpa_s->wnm_mbo_trans_reason_present)
        wpa_msg(wpa_s, MSG_INFO, MBO_TRANSITION_REASON "reason=%u", wpa_s->wnm_mbo_transition_reason);

    if (disallowed_sec && wpa_s->current_bss)
        wpa_bss_tmp_disallow(wpa_s, wpa_s->current_bss->bssid, disallowed_sec, 0);

    return;
fail:
    wpa_printf(MSG_DEBUG, "MBO IE parsing failed (id=%u len=%u left=%zu)", id, elen, len);
}

size_t wpas_mbo_ie_bss_trans_reject(struct wpa_supplicant *wpa_s,
                                    u8 *pos,
                                    size_t len,
...
void wpas_mbo_update_cell_capa(struct wpa_supplicant *wpa_s, u8 mbo_cell_capa)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "MBO: Cellular capability already set to %u", mbo_cell_capa);
        return;
    }
...
struct wpabuf *mbo_build_anqp_buf(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, u32 mbo_subtypes)
{
    ...
    {
        wpa_printf(MSG_INFO, "MBO: " MACSTR " does not support MBO - cannot request MBO ANQP elements from it",
                   MAC2STR(bss->bssid));
        return NULL;
    }
    ...
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, const u8 *sa, const u8 *data, size_t slen)
{
    ...
    switch (subtype)
    {
        case MBO_ANQP_SUBTYPE_CELL_CONN_PREF:
            if (slen < 1)
                break;
            wpa_msg(wpa_s, MSG_INFO, RX_MBO_ANQP MACSTR " cell_conn_pref=%u", MAC2STR(sa), *pos);
            break;
        default:
            wpa_printf(MSG_DEBUG, "MBO: Unsupported ANQP subtype %u", subtype);
            break;
    }
}
#endif
